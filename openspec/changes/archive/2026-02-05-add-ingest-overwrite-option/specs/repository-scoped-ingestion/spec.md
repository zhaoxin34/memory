# Repository-Scoped Ingestion Specification
...
The `ingest` command SHALL support a `--force` option for overwriting existing documents.

#### Scenario: Overwrite existing document in repository

- **WHEN** user runs `memory ingest <path> --repository <name> --force`
- **AND** a document with the same source path exists in the repository
- **THEN** system SHALL delete the existing document, all its chunks, and all embeddings from that repository
- **AND** system SHALL import the new document to the specified repository
- **AND** system SHALL display confirmation that document was overwritten

#### Scenario: Force flag with non-existent document

- **WHEN** user runs `memory ingest <path> --repository <name> --force`
- **AND** no document with the same source path exists in the repository
- **THEN** system SHALL import the document as a new document to the specified repository
- **AND** system SHALL display message indicating new document creation (not overwrite)

#### Scenario: Batch overwrite in repository

- **WHEN** user runs `memory ingest <directory> --recursive --repository <name> --force`
- **THEN** system SHALL overwrite all existing documents in the specified repository
- **AND** system SHALL create new documents for files not already in the repository
- **AND** system SHALL display summary of operations for the repository
